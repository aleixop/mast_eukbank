...
summary_asvs_marine_vs_continental <-
  ...
  mutate(
    perc_n = 100 * n / sum(n),
    type = factor(type, levels = c("marine", "continental", "shared"))
  )
...
shared_asvs_abun_marinecontinental <-
  asvs_marine_vs_continental %>%
  mutate(
    type = factor(type, c("marine", "continental", "shared")),
    tax = if_else(is.na(taxogroup3), taxogroup2, taxogroup3),
    tax = factor(tax, levels = order_all_masts)
  ) %>%
  filter(type == "shared") %>%
...
dev.off()
# Sadly, I reordered the legend manually in Illustrator
```
